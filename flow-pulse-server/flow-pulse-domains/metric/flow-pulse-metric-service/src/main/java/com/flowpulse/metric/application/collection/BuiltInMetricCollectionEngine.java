...
class BuiltInMetricCollectionEngine {
    ...
    BuiltInMetricCollectionEngine(InfrastructureMapper infrastructureMapper,
    ...
    public boolean supports(MetricCollectContext context) {
        ...
        String collector = normalize(firstNotBlank(context.getImplementation().getBuiltInCollector(),
                context.getImplementation().getMetricCode(), ""));
        return collector.startsWith("KAFKA_") || collector.startsWith("SPARK_") || collector.startsWith("ES_");
    }
    ...
    public MetricCollectResult collect(MetricCollectContext context) throws Exception {
        ...
        throw new IllegalStateException("Unsupported technology built-in collector: " + collector);
    }
    ...
    private String resolveInfrastructureId(ResourceMetricConfigEntity config) {
        if (isInfrastructureObjectType(config.getObjectType())) {
