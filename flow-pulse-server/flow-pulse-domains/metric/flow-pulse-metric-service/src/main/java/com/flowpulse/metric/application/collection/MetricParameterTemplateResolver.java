...
public class MetricParameterTemplateResolver {
    ...
    private String sourceValue(PlaceholderResolver resolver, ResolutionContext context) {
        ...
        if ("CURRENT_TIME_FORMATTED".equalsIgnoreCase(resolver.sourceType)) {
            return formatTime(now, resolver.format);
        }
    ...
    private String applyTransform(String value, PlaceholderResolver resolver) {
        String type = defaultText(resolver.transformType, hasText(resolver.format) ? "TIME_FORMAT" : "NONE");
        if ("TIME_FORMAT".equalsIgnoreCase(type)) {
            return applyFormat(value, defaultText(resolver.outputFormat, resolver.format));
        }
        if ("CONCAT".equalsIgnoreCase(type)) {
            return defaultText(resolver.prefix, "") + defaultText(value, "") + defaultText(resolver.suffix, "");
        }
        if ("REGEX_EXTRACT".equalsIgnoreCase(type)) {
            return regexExtract(value, resolver.regex, resolver.groupIndex);
        }
    ...
    private boolean matchesOperator(String value, PlaceholderResolver resolver) {
        String operator = defaultText(resolver.operator, "NONE");
        String compareValue = resolver.compareValue;
        if (!hasText(compareValue) || "NONE".equalsIgnoreCase(operator)) {
    ...
    private String applyFormat(String value, String format) {
        ...
        try {
            return formatTime(Long.parseLong(value), format);
        } catch (NumberFormatException ignored) {
            return defaultText(value, "");
        }
    }

    private String formatTime(long epochMillis, String format) {
        String pattern = hasText(format) ? format : "yyyyMMddHHmmss";
        return DateTimeFormatter.ofPattern(pattern).format(Instant.ofEpochMilli(epochMillis).atZone(ZoneId.systemDefault()));
    }
